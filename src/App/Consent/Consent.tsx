...
import { localModListAtom, consentOverlayDataAtom, tutorialOpenAtom, tutorialPageAtom, DirRestructureItem } from "@/utils/vars";
import { saveConfig, createRestorePoint, refreshRootDir } from "@/utils/fsutils";
...
function Consent() {
	...
	return (
		<motion.div
			...
			<div className="w-180 h-150 bg-background/50 border-border flex flex-col items-center gap-4 p-4 overflow-hidden border-2 rounded-lg">
				...
				<div className="h-100 flex items-center w-full p-0">
					...
					<div className="flex flex-col w-1/2 h-full overflow-x-hidden overflow-y-auto text-gray-300 border rounded-sm">
						{consentOverlayData.to.map((item, index) => (
							<div className={"w-full flex  flex-col"} style={{ backgroundColor: index % 2 == 0 ? "#1b1b1b50" : "#31313150", borderBottom: index == consentOverlayData.to.length - 1 ? "" : "1px solid var(--border)" }}>
