...
import { categoriesAtom, firstLoadAtom, localDataAtom, localItemsAtom, presetsAtom, rootPathAtom, settingsAtom, store } from "./variables";
...
export const apiRoutes = {
	genericTypes: "https://gamebanana.com/apiv11/Mod/Categories?_idGameRow=20357&_sSort=a_to_z&_bShowEmpty=true",
	genericCategories: "https://gamebanana.com/apiv11/Mod/Categories?_idCategoryRow=29524&_sSort=a_to_z&_bShowEmpty=true",
};
...
export async function main() {
	...
	store.set(rootPathAtom, config.dir);
	store.set(presetsAtom, config.presets);
	store.set(localDataAtom, config.data);
	store.set(settingsAtom, config.settings);
	store.set(categoriesAtom, [
		...(await fetch("https://gamebanana.com/apiv11/Mod/Categories?_idCategoryRow=29524&_sSort=a_to_z&_bShowEmpty=true")
			.then((x) => x.json())
			.then((x) => x.filter((x: any) => x._idRow !== 31838))),
		{
			_idRow: 31838,
		...
		{ _special: true, _idRow: 29493, _sName: "Other/Misc", _nItemCount: 75, _nCategoryCount: 0, _sUrl: "https://gamebanana.com/mods/cats/29493", _sIconUrl: "https://images.gamebanana.com/img/ico/ModCategory/6692c90cba314.png" },
	...
	if (!firstLoad) {
		store.set(localItemsAtom, await refreshDir(config.dir, "", config.data));
	}
	// pending hotkeys
}
